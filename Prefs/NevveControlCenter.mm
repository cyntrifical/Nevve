#import "NevveControlCenter.h"

@implementation NevveControlCenterListController

+ (NSString *)hb_specifierPlist {

    return @"NevveControlCenter";
    
}

-(void)listThatDir {

   directoryControlCenterContent = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:@"/Library/Nevve/" error:NULL];
...
- (NSArray *)getData:(id)target {
    ...
    for (NSURL *fileURL in [directoryControlCenterContent filteredArrayUsingPredicate:predicate]) {
...
- (void)checkForFiles {
    ...
    if ([directoryControlCenterContent count] == 0) {

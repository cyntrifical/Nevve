#import "NevveHardwareButtons.h"

@implementation NevveHardwareButtonsListController

+ (NSString *)hb_specifierPlist {

    return @"NevveHardwareButtons";
    
}

-(void)listThatDir {

   directoryHardwareButtonsContent = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:@"/Library/Nevve/" error:NULL];
...
- (NSArray *)getData:(id)target {
    ...
    for (NSURL *fileURL in [directoryHardwareButtonsContent filteredArrayUsingPredicate:predicate]) {
...
- (void)checkForFiles {
    ...
    if ([directoryHardwareButtonsContent count] == 0) {

#import "NevveOtherHardwareActions.h"

@implementation NevveOtherHardwareActionsListController

+ (NSString *)hb_specifierPlist {

    return @"NevveOtherHardwareActions";
    
}

-(void)listThatDir {

   directoryOtherHardwareActionsContent = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:@"/Library/Nevve/" error:NULL];
...
- (NSArray *)getData:(id)target {
    ...
    for (NSURL *fileURL in [directoryOtherHardwareActionsContent filteredArrayUsingPredicate:predicate]) {
...
- (void)checkForFiles {
    ...
    if ([directoryOtherHardwareActionsContent count] == 0) {
